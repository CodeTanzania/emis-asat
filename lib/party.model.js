 ...
 * @todo add party level(Region, Ward etc)
 * @todo add part command(operational, tactical etc)
 * @todo party with multiple parent(e.g Teams, Committee)
 * @todo add title support
 * @todo add members support
 * @todo prevent autopopulation if specific field are selected
 * @todo add country, city(region) and county(district)
 */
...
const { getString, getStrings } = env;
...
const DEFAULT_PARTY_OWNERSHIP = getString('DEFAULT_PARTY_OWNERSHIP', 'Other');
...
const PARTY_OWNERSHIP = getStrings('PARTY_OWNERSHIP', DEFAULT_PARTY_OWNERSHIP);
...
/**
 * @name filters
 * @description prepare party filters
 * @param {function} done callback to invoke on success or error
 * @since 0.1.0
 * @version 1.0.0
 * @static
 * @public
 */
PartySchema.statics.filters = function filters(criteria, done) {
  //ensure criteria
  // TODO const _criteria = _.merge({}, criteria);

  /* @todo impement database filter */
  //zero filters
  let filters = ([
    { group: 'phases', data: DISASTER_PHASES },
    { group: 'types', data: PARTY_TYPES },
    { group: 'ownership', data: PARTY_OWNERSHIP }
    /* @todo title, roles & functions filters */
  ]);
  ...
  filters = _.map(filters, function (filter) {
    ...
    //prepare filter
    filter.data = _.map(filter.data, function (data) {
      return {
        name: data,
        count: 0
      };
...
/**
 * @name dashboard
 * @description prepare party dashboard
 * @param {function} done callback to invoke on success or error
 * @since 0.1.0
 * @version 1.0.0
 * @static
 * @public
 */
PartySchema.statics.dashboard = function dashboard(criteria, done) {
  //ensure criteria
  ...
  /* @todo refactor to use aggregation */
  //prepare the dashboard
  async.parallel({

    filters: function runFilters(next) {
      Party.filters(_criteria, next);
    },

    query: function runQuery(next) {
      Party.get(_criteria, next);
    }

  }, function onDashboardReady(error, results) {
    if (!error) {
      results = _.merge({}, results.query, { filters: results.filters });
    }
    done(error, results);
  });
